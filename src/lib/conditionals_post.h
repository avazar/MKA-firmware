...
// LCD
#define HAS_LCD         (ENABLED(NEWPANEL) || ENABLED(NEXTION) || ENABLED(NEXTION_HMI))
...
#define HAS_SD_SUPPORT       (ENABLED(SDSUPPORT))
  ...
  #define MAT_NAME_SIZE 40
  #if ENABLED(SD_SECOND_CARD)
	#define SD_COUNT 2
  #else
    #define SD_COUNT 1
  #endif
...
#endif
#if ENABLED(SDSUPPORT) && ENABLED(ARDUINO_ARCH_SAM) && DISABLED(SDCARD_SORT_ALPHA)
 ...
 */
  ...
  #undef SD_DETECT_PIN_1
  #define SD_DETECT_PIN_1 -1
#endif
...
#define HEATER_USES_MAX       (ENABLED(SUPPORT_MAX6675) || ENABLED(SUPPORT_MAX31855) || ENABLED(SUPPORT_MAX31865))
...
#define HEATER_IDLE_HANDLER (ENABLED(ADVANCED_PAUSE_FEATURE) || ENABLED(PROBING_HEATERS_OFF) || ENABLED(NEXTION_HMI))
...
/**
 * Set a flag for the nozzle calibration probe (uses Z_PROBE_PIN)
 */
#define HAS_CALIBRATION_PROBE (ENABLED(CALIBRATION_PROBE) && HAS_Z_PROBE_PIN)
...
#define USE_GCODE_SUBCODES (ENABLED(G38_PROBE_TARGET) || ENABLED(M28_FAST_UPLOAD))
